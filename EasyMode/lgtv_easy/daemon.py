    ...
    def _start_sleep_watcher(self) -> None:
        ...
        try:
            ...
            if name and name != "none":
                ...
                # No OS hook here: say so, so "the TV never sleeps with the PC"
                # always has a line in the log explaining itself.
                self.logger.info(
            ...
            # while the setting still reads ON. A registration bug did exactly
            # that on every Windows machine for months, unnoticed, because this
            # line was invisible.
            self.logger.warning(
